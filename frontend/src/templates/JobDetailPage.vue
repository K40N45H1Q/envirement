...
const job = ref(null), loading = ref(false), submitting = ref(false)
const error = ref(''), applyStatus = ref('')
const brokenLogo = ref(false), brokenBanner = ref(false)
const applyModal = ref(null), bannerModal = ref(null), resume = ref(null)
...
const languages = computed(() => job.value?.languages?.filter(({ name, level }) => name && level)
  .map(item => ({ ...item, name: language.value === 'en' ? localizeLanguage(item.name) : item.name })) || [])
...
const loadJob = async () => {
  loading.value = true
  error.value = ''
  brokenLogo.value = brokenBanner.value = false
  try { job.value = normalizeJob(await getJob(route.params.id)) }
  catch { job.value = null; error.value = t('jobDetailPage.errors.notFound') }
  finally { loading.value = false }
}

const submitApplication = async () => {
  applyStatus.value = ''
  if (!user.value) return applyStatus.value = t('jobDetailPage.apply.loginRequired')
...
onMounted(async () => {
  ...
  if (user.value) try {
    ...
    Object.assign(form.value, { name: profile.first_name || '', surname: profile.last_name || '', phone: profile.phone || '' })
  } catch {}
  loadJob()
})
onBeforeUnmount(() => { document.body.style.overflow = previousOverflow.value })
        ...
        <section class="content">
          ...
          <aside class="panel sidebar">
            <RouterLink class="side-footer" to="/jobs"
              >← {{ t('jobDetailPage.allJobs') }}</RouterLink
            >
      ...
      <form class="modal-card" @submit.prevent="submitApplication">
        ...
        <footer>
          <button class="primary" :disabled="submitting">
            {{ t('jobDetailPage.form.submit') }}{{ submitting ? '…' : '' }}
          </button>
        </footer>
      </form>
    </dialog>
